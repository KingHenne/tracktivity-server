...
public class ActivityServiceTest {
	...
	public void importMultipleTracks() {
		...
		assertEquals(String.format("The track count has not been increased by %d.", COUNT), countBefore + COUNT,
	...
	public void testAddNewTrack() {
		...
		assertEquals("The track count has not been increased by 1.", countBefore + 1, activityService.getTrackCount());
	}
	...
	public void testDatabaseFetchingPerformance() {
		...
		assertNotNull(track);
		assertTrue(String.format("fetching took way too long: %d ms", millis), millis < 500);
	}
	...
	public void testLengthCalculationPerformance() {
		...
		long end = System.currentTimeMillis();
		long millis = end - start;
		assertTrue("length was 0", length > 0);
		assertTrue(String.format("calculation took way too long: %d ms", millis), millis < 100);
	}
	...
	public void testRetrieveActivityWithTrack() {
		...
		for (int i = 0; i < pointsCount - 1; i++) {
			...
			assertTrue(p2.toString(), before || atTheSameTime);
			double d = GeometryUtils.getInstance().getDistance(p1, p2);
			assertTrue(String.format("A distance of %.2fm between two points seems too high.", d), d < 100.0);
		}
	}
}
